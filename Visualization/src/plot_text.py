...
STOPWORDS = {
    # 1. Paper Structure
    'paper', 'article', 'abstract', 'introduction', 'conclusion', 'section', 
    'figure', 'table', 'reference', 'author', 'authors', 'et al',
    
    # 2. Core Concepts
    'model', 'learning', 'algorithm', 'data', 'dataset', 'datasets', 'data set', 
    'embeddings', 'word', 'words', 'language', 'text', 'feature', 'features', 'information',
    
    # 3. Methodology
    'approach', 'method', 'proposed method', 'system', 'framework', 'design', 
    'structure', 'theory', 'type', 'types', 'process', 'analysis',
    
    # 4. Context
    'result', 'results', 'task', 'performance', 'problem', 'challenge', 'study', 
    'research', 'work', 'context', 'concept', 'concepts', 'state art', 'state-art', 
    'state of the art', 'state', 'art', 'discourse', 'future', 'learned', 'effectiveness',
    
    # 5. Common Verbs
    'propose', 'proposed', 'using', 'used', 'use', 'based', 'identify', 
    'achieve', 'achieves', 'show', 'provide', 'present', 
    
    # 6. Evaluative & Descriptive Modifiers
    'efficient', 'effective', 'novel', 'simple', 'general', 'total', 'high', 
    'low', 'highly', 'easily', 'significantly', 'significant', 'new', 
    'challenging', 'extensive', 'different', 'fine', 'popular',
    
    # 7. Connectors, Pronouns & Quantifiers
    'well', 'various', 'given', 'within', 'however', 'also', 'across', 
    'one', 'two', 'many', 'set', 'time', 'long', 'futhermore', 'moreover',
...
class ArxivUltimateVisualizer:
    ...
    def __init__(
        self, 
        data_dir: str = "Visualization/data", 
        plot_dir: str = "Visualization/plot"
    ) -> None:
        """
        ...
        self.period_order: List[str] = [
            '1991_1998', '1999_2006', '2007_2014', '2015_2021'
        ]
    ...
    def load_all_data(self) -> bool:
        ...
        try:
            ...
            self.data['embed'] = np.load(
                f"{self.data_dir}/embeddings.npy"
            )
            self.data['tfidf'] = sparse.load_npz(
                f"{self.data_dir}/tfidf_matrix.npz"
            )
            self.data['ngram'] = sparse.load_npz(
                f"{self.data_dir}/ngram_matrix.npz"
            )
            self.data['sim'] = pd.read_csv(
                f"{self.data_dir}/similarity_matrix.csv", 
                index_col=0
            )
            self.data['df_cat'] = pd.read_csv(
                f"{self.data_dir}/keywords_by_category.csv"
            )
            self.data['df_per'] = pd.read_csv(
                f"{self.data_dir}/keywords_by_period.csv"
            )
            vocab_path = f"{self.data_dir}/tfidf_vocab.pkl"
            self.data['vocab'] = pickle.load(open(vocab_path, "rb")) if os.path.exists(vocab_path) else None
    ...
    def plot_2_cloud(self) -> None:
        ...
        word_freq = {
            ...
            if str(row['word']).lower() not in STOPWORDS 
    ...
    def plot_3_embedding(self) -> None:
        ...
        plt.savefig(f"{self.plot_dir}/03_embeddings_clustering.png", bbox_inches='tight')
        plt.close()
    ...
    def plot_4_trajectory(self) -> None:
        ...
        def get_axis_labels(comp: np.ndarray) -> Tuple[str, str]:
            ...
            low_terms = [self.data['vocab'][i] for i in idx if valid(self.data['vocab'][i])]
            high_terms = [self.data['vocab'][i] for i in idx[::-1] if valid(self.data['vocab'][i])]
            return low_terms[0], high_terms[0]
            
        x_labels, y_labels = get_axis_labels(svd.components_[0]), get_axis_labels(svd.components_[1])
        ...
        for period in self.period_order:
            ...
            plt.scatter(
                ...
                c=self.period_colors[period], s=2, alpha=0.1
            )
        ...
        for period in res.index:
            plt.scatter(
                res.loc[period, 'x'], res.loc[period, 'y'], 
                s=450, color=self.period_colors[period], 
                edgecolors='white', lw=3, label=period, zorder=12
            )
    ...
    def plot_6_ngram_density_complexity(self) -> None:
        ...
        sns.boxplot(
            data=df, x='period', y='ngram_density', 
            order=self.period_order, hue='period', 
            palette=self.period_colors, showfliers=False, legend=False
        )
    ...
    def plot_7_fingerprint(self) -> None:
        ...
        pivot_df = df_cat[df_cat['word'].isin(top_overall)].pivot(
            index='word', columns='category', values='score'
        ).fillna(0)
        
        plt.figure(figsize=(12, 11))
        sns.heatmap(
            pivot_df, annot=True, cmap="YlGnBu", 
            fmt=".2f", cbar_kws={'label': 'TF-IDF Weight'}
        )
        plt.title("Domain Fingerprint: Technology Focus Across Fields", pad=20, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_8_evolution(self) -> None:
        ...
        pivot_per = df_per.pivot(index='word', columns='period', values='score').fillna(0)
        ...
        plt.title("Tech Transition: Rise and Decay of AI Concepts", pad=20, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_9_phrases(self) -> None:
        ...
        for i, period in enumerate(self.period_order):
            subset = self.data['df'][self.data['df']['period'] == period]['clean_text'].dropna()
            cv = CountVectorizer(ngram_range=(2, 3), max_features=50, stop_words='english')
            ...
            try:
                ...
                axes[i].set_title(f"Key Terms: {period}", color=self.period_colors[period], fontweight='bold')
