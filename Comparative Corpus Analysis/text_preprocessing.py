...
def keywords_by_period(df):
    ...
    pd.DataFrame(rows).to_csv("keywords_by_period.csv", index=False)
...
def keywords_by_category(df):
    ...
    pd.DataFrame(rows).to_csv("keywords_by_category.csv", index=False)
...
def ngram_representation(df):
    ...
    save_npz("ngram_matrix.npz", X_ngram)
...
def generate_embeddings(df):
    ...
    np.save("embaddings.npy", X_embed)
